...
export const parseHeaders = (headers) =>
  headers.reduce((combined, _header) => {
    const header = isFunction(_header) ? _header() : _header;

    if (!isObject(header)) {
      console.warn(
        `Expecting an object or a function that returns an object for the header but got ${typeof header}`
      );
      return {...combined};
    }
...
export const prepareHeaders = (...headers) =>
  new Headers(parseHeaders(headers));
